...
describe('MyGovernor Contract', function () {
  ...
  describe('MyGovernorの初期化テスト', function () {
    it('ガバナンストークンが正しく設定されているかのテスト', async function () {
    ...
    it('TimelockControllerが正しく設定されているかのテスト', async function () {
    ...
    it('投票遅延の初期設定が正しく行われているかのテスト', async function () {
    ...
    it('投票期間の初期設定が正しく行われているかのテスト', async function () {
      const { myGovernor } = await loadFixture(deployFixture);
    ...
    it('投票閾値の初期設定が正しく行われているかのテスト', async function () {
      const { myGovernor } = await loadFixture(deployFixture);
  ...
  describe('提案作成から実行機能のテスト', function () {
    it('権限がある人が正しく提案を作成し、実行まで完了できるかのテスト', async function () {
      ...
      // 提案実行前に、対象のアカウントは10トークンのみ保持していることを確認
      const myERC20WithAuthorized = myERC20.connect(authAccount);
      expect(await myERC20WithAuthorized.balanceOf(await authAccount.getAddress())).to.equal(
        1000000
      ); // 1000000 initial
      ...
      // 委任が成功したことを確認
      expect(await myERC20WithAuthorized.delegates(await authAccount.getAddress())).to.equal(
      ...
      const proposal = {
        ...
        values: [0],
        calldatas: [
          myERC20.interface.encodeFunctionData('mint', [await authAccount.getAddress(), 1000000]),
        ],
        description: 'mint 10 tokens to authAccount',
      };

      // authAccountを使用してmyGovernorに接続
      ...
      ); // 1 is the enum value for "Active"
      ...
      // 想定通りの投票状況になっていることを確認（賛成1000000、反対0、棄権0）
      ...
      // 5 is the enum value for "Queued"
      ...
      // 7 is the enum value for "Executed"

      // 提案が実行され、想定通り、対象のアカウントに10トークンが追加され、20トークンになっていることを確認
      expect(await myERC20WithAuthorized.balanceOf(await authAccount.getAddress())).to.equal(
        2000000
      ); // 2000000 initial + 1000000 minted
    });
